...
class DiscordNotifier:
    ...
    def __init__(self, config=None):
        ...
        self.mention_on_critical = self.config.get(
            "notifications.discord.mention_on_critical", "@everyone"
        )
    ...
    def send_message(
        self, content: str = None, embeds: List[DiscordEmbed] = None
    ) -> bool:
    ...
    def send_cost_alert(
        ...
        # Create embed
        embed = DiscordEmbed(
            title=f"{emoji} AWS Cost Alert: {alert_type.title()}", color=color
        )
    ...
    def send_daily_summary(self, summary: Dict) -> bool:
        ...
        # Create embed
        embed = DiscordEmbed(
            title="📊 AWS Daily Cost Summary", color="03b2f8"  # Blue color
        )

        embed.add_embed_field(name="Today", value=f"${today_cost:.2f}", inline=True)
        embed.add_embed_field(
            name="Yesterday", value=f"${yesterday_cost:.2f}", inline=True
        )
        embed.add_embed_field(
            name="Daily Change", value=f"{trend_emoji} {change:+.1f}%", inline=True
        )
        embed.add_embed_field(name="This Week", value=f"${week_cost:.2f}", inline=True)
        embed.add_embed_field(
            name="This Month", value=f"${month_cost:.2f}", inline=True
        )
        embed.add_embed_field(
            name="Avg Daily", value=f"${summary['avg_daily_cost']:.2f}", inline=True
        )
        ...
        for period, threshold_data in summary.get("thresholds", {}).items():
            if threshold_data.get("exceeded"):
                ...
                warnings.append(f"⚠️ {period.title()}: {pct:.0f}% of budget")
    ...
    def send_anomaly_alert(self, anomalies: List[Dict]) -> bool:
        ...
        for i, anomaly in enumerate(anomalies[:3], 1):
            ...
            embed.add_embed_field(name=f"Anomaly #{i}", value=field_value, inline=True)

        if len(anomalies) > 3:
            embed.set_footer(
                text=f"...and {len(anomalies) - 3} more anomalies | AWS Cost Sentinel"
            )
        else:
            embed.set_footer(text="AWS Cost Sentinel")
